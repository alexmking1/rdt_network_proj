...
class B:
    ...
    def B_input(self, pkt):
        ...
        #print("b SIDE ::: "+ "[pkt.acknum => " + str(pkt.acknum) + "]---[pkt.acknum => " + str(pkt.acknum) + "]<---- end") 
        ...
        # CASE2: rcv 'corrupt pkt' or 'UNexpected seq' --- [box2 of FSM]
            ...
            # EDGE CASES TO HANDLE HERE: 1.what if the first packet is lost OR [DONE] // 2.what if the numbers do not match? 
            if self.expectedSeqNum == 0: # EDGE CASE if FIRST packet is lost, we want to send the original val back
                 send_ack("B", (self.expectedSeqNum))
            elif self.expectedSeqNum > 0: # GENERAL CASE...
